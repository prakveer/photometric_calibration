    ...
    m_response_estimate()
{
    ...
    total_tracked_features = 0;
    total_exposure_time =0;
}
cv::Mat Database::getcorrectedImage(){
...
void Database::visualizeTracking()
{
    ...
    //std::cout<<" over_exposed: "<<over_exp/total<<std::endl;
...
void Database::visualizeRapidExposureTimeEstimates(double exponent)
{
    ...
    if(gt_exp_times.size() == estimated_exp_times.size()){
        ...
        //std::cout<<" alignment_alpha "<<alignment_alpha<<std::endl;
    }
    else
    {
    ...
    if(gt_exp_times.size() == estimated_exp_times.size())
    {
        for(int i = 0;i < nr_frames_to_vis-1;i++)
        {
            ...
            cv::line(exposure_vis_image, cv::Point(draw_spacing*i,drawing_y_gt_exp_1), cv::Point(draw_spacing*(i+1),drawing_y_gt_exp_2), cv::Scalar(255,0,0));
        }
    }   
