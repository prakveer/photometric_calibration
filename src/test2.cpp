...
public:
  ...
  void extractimage(cv::Mat &new_image)
  {
...
{   
    ...
    Listener listener(cv::Size(run_settings->image_width, run_settings->image_height));
    image_transport::ImageTransport it(nh);
    image_transport::Subscriber sub = it.subscribe("camera/image", 10, &Listener::imagecallback, &listener); 
    image_transport::Publisher pub = it.advertise("camera/image_corrected", 1);
 

    ros::Rate loop_rate(25);

    // setting up the optimization pipeline
    ...
    int i=1;
    double avg_exposure = 0;

    //Run over all input images, track the new image, estimate exposure time and optimize other parameters in the background
    while(nh.ok())        
    {
        // see if a new image is in the buffer, if yes,  process it
        if(listener.getimageflag())
        {
            cv::Mat new_image;
            listener.extractimage(new_image);
            ...
            avg_exposure = database.total_exposure_time / database.total_tracked_features;

            // Rapid exposure time estimation (+ time the result)
            double exposure_time = exposure_estimator.estimateExposureTime();
            std::cout<<std::setw(10)<<" exp_est: "<<exposure_time <<std::setw(10) <<" prediction: "<<(gt_exp_time/avg_exposure)<<std::setw(10)<<" gt: "<<gt_exp_time<<std::setw(10)<<" avg: "<<avg_exposure<<std::endl;

            exposure_time = gt_exp_time/avg_exposure;                
...
/**IMPORTANT: BETTER TO CLONE IMAGE_TOPIC IF YOU HAPPEN TO EDIT/USE THE PUBLISHED IMAGE */
            //publish the corrected frame:
            cv::Mat corrected_image = database.m_tracked_frames.at(database.m_tracked_frames.size()-1).m_image_corrected;
            sensor_msgs::ImagePtr msg;
            msg = cv_bridge::CvImage(std_msgs::Header(), "mono8", corrected_image).toImageMsg();
            pub.publish(msg);


            ////Visualize tracking
            if((i%run_settings->visualize_cnt == 0)  &&  debug)
            ...
            //if  Optimization is still running, don't do anything and keep tracking
            if(!is_optimizing)
            {            
 ...
 int run_batch_calibration(Settings *run_settings,std::vector<double> gt_exp_times)
{
    ...
    for(int i = run_settings->start_image_index; i < num_images && (run_settings->end_image_index < 0 || i < run_settings->end_image_index); i++)
    {
        ...
        //avg_exposure = (i* 0.5*avg_exposure + 1.5*gt_exp_time)/(0.5*i+1.5);
...
{   
    ...
    run_settings.exposure_gt_file = "/home/prakveer/Desktop/online_photometric_calibration/sequence_30/times.txt";
    ...
    {    
        ...
        gt_exp_times.at(k) = 3*gt_exp_times.at(k);
