...
void Tracker::trackNewFrame(cv::Mat input_image, double gt_exp_time)
{
    ...
    // Correct the input image based on the current response and vignette estimate and exposure time
    // Todo: move to class member
    cv::Mat corrected_frame = input_image.clone();
    photometricallyCorrectImage(corrected_frame, gt_exp_time);
    ...
    for(int i = 0;i < feature_locations.size();i++)
    {
        ...
        if(tracked_point_status.at(i) == 0 || validity_vector.at(i) == 0){
            ...
            m_database->total_exposure_time -= m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_features.at(i)->m_cumulative_exposure_time;
            m_database->total_tracked_features -= m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_features.at(i)->m_cumulative_frames;
            continue;
        }
        
        m_database->total_tracked_features++;
        m_database->total_exposure_time +=gt_exp_time;
        ...
        f->m_cumulative_exposure_time = gt_exp_time + m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_features.at(i)->m_cumulative_exposure_time;
        f->m_cumulative_frames =1 + m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_features.at(i)->m_cumulative_frames;
    ...
    for(int p = 0;p < new_feature_locations.size();p++)
    {
        ...
        f->m_cumulative_exposure_time = gt_exp_time;
        f->m_cumulative_frames =1;
...
void Tracker::photometricallyCorrectImage(cv::Mat &corrected_frame, double gt_exp_time)
{   int hyper_radiance = 0;
    int hypo_radiance = 0;
    float total =0;
    ...
    //std::cout<<" hyper_radiance "<< hyper_radiance/total<<" hypo_radiance "<<hypo_radiance/total<<std::endl;
    /*
